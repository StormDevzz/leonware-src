...
{
    private static final AutoEatModule instance;
    private final SliderSetting hunger;
    private final MultiBooleanSetting blacklist;
    private boolean eating;
    private int prevSlot;
    
    public AutoEatModule() {
        this.hunger = new SliderSetting("Hunger").value(14.0f).range(0.0f, 20.0f).step(1.0f);
        this.blacklist = new MultiBooleanSetting("Blacklist").value(new BooleanSetting("Golden Apple").value(true), new BooleanSetting("Chorus").value(true), new BooleanSetting("Rotten Flesh").value(true));
    ...
    public void onEvent() {
        final EventListener updateEvent = UpdateEvent.getInstance().subscribe(new Listener<UpdateEvent>(event -> {
            if (AutoEatModule.mc.field_1724 == null || AutoEatModule.mc.field_1687 == null) {
                ...
                if (AutoEatModule.mc.field_1724.method_7344().method_7586() <= (int)(float)this.hunger.getValue()) {
                    final boolean found = this.isHandGood(class_1268.field_5808) || this.isHandGood(class_1268.field_5810) || this.switchToFood();
                    if (!found) {
                        if (this.eating) {
                            this.stopEating();
                        }
    ...
    private boolean switchToFood() {
        for (int i = 0; i < 9; ++i) {
            final class_1799 stack = AutoEatModule.mc.field_1724.method_31548().method_5438(i);
            if (this.isValidFood(stack)) {
        ...
        while (i < 36) {
            ...
            if (!this.isValidFood(stack)) {
                ...
                final int targetSlot = this.findNonSwordHotbarSlot();
                if (targetSlot == -1) {
                    return false;
                }
                this.prevSlot = AutoEatModule.mc.field_1724.method_31548().field_7545;
    ...
    private boolean isHandGood(final class_1268 hand) {
        final class_1799 stack = (hand == class_1268.field_5808) ? AutoEatModule.mc.field_1724.method_6047() : AutoEatModule.mc.field_1724.method_6079();
        return this.isValidFood(stack);
    }
    
    private boolean isValidFood(final class_1799 stack) {
        ...
        return (!this.blacklist.isEnabled("Golden Apple") || (item != class_1802.field_8463 && item != class_1802.field_8367)) && (!this.blacklist.isEnabled("Chorus") || item != class_1802.field_8233) && (!this.blacklist.isEnabled("Rotten Flesh") || item != class_1802.field_8511);
    }
    
    private int findNonSwordHotbarSlot() {
        final int current = AutoEatModule.mc.field_1724.method_31548().field_7545;
        for (int i = 0; i < 9; ++i) {
            if (i != current) {
                final class_1799 stack = AutoEatModule.mc.field_1724.method_31548().method_5438(i);
