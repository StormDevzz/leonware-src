...
{
    private static final ElytraSwapModule instance;
    private final BindSetting swapKey;
    private final BindSetting launchKey;
    ...
    public ElytraSwapModule() {
        this.swapKey = new BindSetting("Swap key").value(-999);
        this.launchKey = new BindSetting("Launch key").value(-999);
        this.legit = new BooleanSetting("Legit").value(false).setVisible(() -> this.launchKey.getValue() != -999);
    ...
    @Override
    public void onEvent() {
        final EventListener tickEvent = TickEvent.getInstance().subscribe(new Listener<TickEvent>(event -> this.handleMainLogic(!SlownessManager.isEnabled())));
        final EventListener updateEvent = UpdateEvent.getInstance().subscribe(new Listener<UpdateEvent>(event -> this.handleMainLogic(SlownessManager.isEnabled())));
        this.addEvents(tickEvent, updateEvent);
    }
    
    private void handleMainLogic(final boolean slow) {
        this.handleFireworkLaunch(slow);
        this.handleChestplateSwap(slow);
    }
    
    public void handleFireworkLaunch(final boolean tick) {
        if (tick || !ElytraSwapModule.mc.field_1724.method_6128()) {
            return;
        }
        this.itemUsage.handleUse(this.launchKey.getValue(), this.legit.getValue());
    }
    
    public void handleChestplateSwap(final boolean tick) {
        if (tick) {
            return;
        }
        if (KeyStorage.isPressed(this.swapKey.getValue())) {
    ...
    public void swapChestplate() {
        ...
        if (InventoryUtil.hasElytraEquipped()) {
            final int slot = this.slots();
            if (slot != -1) {
                if (slot >= 0 && slot <= 8) {
                    InventoryUtil.swapSlotsFull(6, slot);
                }
                else if (slot >= 36 && slot <= 44) {
                    final int hotbarSlot = slot - 36;
    ...
    private int findBestSlotFor(final class_1792... items) {
        for (final class_1792 item : items) {
            final int slot = InventoryUtil.findItem(item);
            if (slot != -1) {
                return slot;
            }
        }
    ...
    public static ElytraSwapModule getInstance() {
        return ElytraSwapModule.instance;
    }
