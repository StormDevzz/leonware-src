...
{
    private static final ItemSwapModule instance;
    private final BindSetting swapKey;
    private final ModeSetting firstItem;
    private final ModeSetting secondItem;
    private boolean swapping;
    
    public ItemSwapModule() {
        this.swapKey = new BindSetting("Swap key").value(-999);
        this.firstItem = new ModeSetting("First item").value("Shield").values("Shield", "GApple", "Totem", "Ball");
        this.secondItem = new ModeSetting("Second item").value("GApple").values("Shield", "GApple", "Totem", "Ball");
        this.swapping = false;
    ...
    public void onEvent() {
        final EventListener keyEvent = KeyEvent.getInstance().subscribe(new Listener<KeyEvent.KeyEventData>(event -> {
            if (event.key() == this.swapKey.getValue() && event.action() == 1 && ItemSwapModule.mc.field_1755 == null) {
                this.swapping = true;
            }
            return;
        }));
    ...
    private void performSwap() {
        ...
        final class_1792 item = this.getItem();
        if (item == null) {
    ...
    private class_1792 getItem() {
        final class_1792 primary = this.getItemByMode(this.firstItem.getValue());
        final class_1792 secondary = this.getItemByMode(this.secondItem.getValue());
        return (ItemSwapModule.mc.field_1724.method_6079().method_7909() == primary) ? secondary : primary;
    }
    
    private void swap(final int slot) {
    ...
    private class_1792 getItemByMode(final String name) {
    ...
    public static ItemSwapModule getInstance() {
        return ItemSwapModule.instance;
    }
    
    static {
        instance = new ItemSwapModule();
    }
}
