...
{
    private static final SlimeFlightModule instance;
    private int slimeJumpCooldown;
    private boolean startSetPitch;
    
    public SlimeFlightModule() {
    ...
    public void onEvent() {
        ...
        final EventListener motionEvent = MotionEvent.getInstance().subscribe(new Listener<MotionEvent.MotionEventData>(event -> this.handleSlimeLogic()));
    ...
    private void handleSlimeLogic() {
        ...
        final class_2338 playerPos = SlimeFlightModule.mc.field_1724.method_24515();
        final boolean slimeNearby = this.isSlimeAdjacent(playerPos);
        if (!slimeNearby || !SlimeFlightModule.mc.field_1724.field_5976 || SlimeFlightModule.mc.field_1724.method_18798().field_1351 <= -1.0) {
            return;
        }
        final class_239 hitResult = SlimeFlightModule.mc.field_1765;
        if (!(hitResult instanceof class_3965)) {
            return;
        }
        final class_3965 blockHitResult = (class_3965)hitResult;
        if (SlimeFlightModule.mc.field_1687.method_8320(blockHitResult.method_17777()).method_26215()) {
            return;
        }
        final int slimeSlot = InventoryUtil.findItem(class_1802.field_8828, true);
        if (slimeSlot == -1) {
            return;
        }
    ...
    private boolean isSlimeAdjacent(final class_2338 pos) {
    ...
    public static SlimeFlightModule getInstance() {
        return SlimeFlightModule.instance;
    }
    
    static {
        instance = new SlimeFlightModule();
    }
}
