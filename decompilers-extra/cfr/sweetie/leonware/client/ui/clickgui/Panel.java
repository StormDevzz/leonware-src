...
extends UIComponent {
    ...
    public boolean charTyped(char chr, int modifiers) {
        for (ModuleComponent mc : this.moduleComponents) {
            if (!mc.charTyped(chr, modifiers)) continue;
            return true;
        }
        return false;
    }
    ...
    public void mouseClicked(double mouseX, double mouseY, int button) {
        if (this.inPanel(mouseX, mouseY)) {
            for (ModuleComponent module : this.moduleComponents) {
                if (!MouseUtil.isHovered(mouseX, mouseY, (double)module.getX(), (double)module.getY(), (double)module.getWidth(), (double)module.getHeight())) continue;
    ...
    private void renderThings(class_332 context, int mouseX, int mouseY, float delta) {
        ...
        float fontSize = this.getHeaderHeight() * 0.475f;
        float moduleY = 0.0f;
        ...
        for (ModuleComponent module : this.moduleComponents) {
            ...
            if (openAnim > 0.0f) {
                ...
                for (SettingComponent setting : module.getSettings()) {
                    ...
                    if (!((double)visibleAnim > 0.0)) continue;
                    settingOffset += (setting.getHeight() + this.gap()) * visibleAnim;
                }
                module.setHeight(module.getDefaultHeight() + ((settingOffset *= openAnim) + this.gap()) * openAnim);
            } else {
            ...
            module.setY((float)((double)this.getY() + this.scrollAnimation.getValue() + (double)moduleY + (double)this.getHeaderHeight()));
    ...
    public boolean inPanel(double mouseX, double mouseY) {
        return MouseUtil.isHovered(mouseX, mouseY, (double)this.getX(), (double)(this.getY() + this.getHeaderHeight()), (double)this.getWidth(), (double)(this.getHeight() - this.getHeaderHeight()));
    }
